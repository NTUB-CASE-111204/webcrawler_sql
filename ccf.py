...
for name in sel:
        ...
        if a_name =='':      #跳過空白的資料
            ...
            if(a_name.count("'") >= 1):  #標題裡有'符號
                ...
                cursor.execute("UPDATE public.brand SET ccf = %s where b_name like '%s'"%(True, a_name))   #更新資料ccf為True
                #print("update///" + a_name)
            else:
                cursor.execute("UPDATE public.brand SET ccf = %s where b_name = '%s'"%(True, a_name))  #更新資料ccf為True
                #print("found/" + a_name)
        else:
            print(a_name)
